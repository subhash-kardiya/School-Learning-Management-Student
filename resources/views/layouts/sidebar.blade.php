<div class="sidebar" id="sidebar-accordion">
    ...
    @canany(['exam_type', 'exam_schedule', 'marks_entry'])
        ...
        <div class="collapse {{ $examsActive ? 'show' : '' }}" id="submenu-exams" data-bs-parent="#sidebar-accordion">
            @can('exam_type')
                <a href="{{ route($examTypeRoute) }}"
                    class="ps-5 submenu {{ request()->routeIs('exams.type*', 'teacher.exams.type*') ? 'current-page' : '' }}">
    ...
    @can('result_view')
        <a href="{{ route($resultsRoute) }}"
            class="d-flex align-items-center {{ request()->routeIs('results.*', 'teacher.results.*') ? 'current-page' : '' }}">
            <span>
