...
use App\Http\Controllers\ContextFilterController;
...
// Academic Year CRUD (uses resource routes)
    ...
    Route::middleware(['permission:exam_type'])->group(function () {
        Route::get('/exams/type', [ExamController::class, 'type'])->name('exams.type');
        Route::post('/exams/type', [ExamController::class, 'storeType'])->name('exams.type.store');
    });
    Route::middleware(['permission:exam_schedule'])->group(function () {
        Route::get('/exams/schedule', [ExamController::class, 'schedule'])->name('exams.schedule');
        Route::post('/exams/schedule', [ExamController::class, 'storeSchedule'])->name('exams.schedule.store');
    });
    Route::middleware(['permission:marks_entry'])->group(function () {
        Route::get('/exams/marks', [ExamController::class, 'marks'])->name('exams.marks');
        Route::post('/exams/marks', [ExamController::class, 'saveMarks'])->name('exams.marks.store');
    });
...
Route::middleware(['role:teacher'])->prefix('teacher')->group(function () {
    ...
    Route::middleware(['permission:timetable.view_own'])->group(function () {
        Route::get('/timetable', [TimetableController::class, 'teacherIndex'])->name('teacher.timetable');
        Route::get('/timetable/data', [TimetableController::class, 'teacherData'])->name('teacher.timetable.data');
    });

    Route::middleware(['permission:attendance_mark'])->group(function () {
        Route::get('/attendance/mark', [AttendanceController::class, 'markForm'])->name('teacher.attendance.mark');
        Route::post('/attendance/mark', [AttendanceController::class, 'markSave'])->name('teacher.attendance.mark.save');
        Route::get('/attendance/grid', [AttendanceController::class, 'grid'])->name('teacher.attendance.grid');
        Route::post('/attendance/update-cell', [AttendanceController::class, 'updateCell'])->name('teacher.attendance.update.cell');
    });

    Route::middleware(['permission:homework_create'])->group(function () {
        Route::get('/homework/create', [HomeworkController::class, 'create'])->name('teacher.homework.create');
        Route::post('/homework/create', [HomeworkController::class, 'store'])->name('teacher.homework.store');
    });
    Route::middleware(['permission:homework_list'])->group(function () {
        Route::post('/homework/{id}/toggle-status', [HomeworkController::class, 'toggleStatus'])->name('teacher.homework.toggle.status');
        Route::delete('/homework/{id}', [HomeworkController::class, 'destroy'])->name('teacher.homework.destroy');
        Route::get('/homework/list', [HomeworkController::class, 'list'])->name('teacher.homework.list');
    });
    Route::middleware(['permission:homework_submission'])->group(function () {
        Route::get('/homework/{id}/submission-report', [HomeworkController::class, 'submissionReport'])->name('teacher.homework.submission.report');
    });

    Route::middleware(['permission:exam_type'])->group(function () {
        Route::get('/exams/type', [ExamController::class, 'type'])->name('teacher.exams.type');
        Route::post('/exams/type', [ExamController::class, 'storeType'])->name('teacher.exams.type.store');
    });
    Route::middleware(['permission:exam_schedule'])->group(function () {
        Route::get('/exams/schedule', [ExamController::class, 'schedule'])->name('teacher.exams.schedule');
        Route::post('/exams/schedule', [ExamController::class, 'storeSchedule'])->name('teacher.exams.schedule.store');
    });
    Route::middleware(['permission:marks_entry'])->group(function () {
        Route::get('/exams/marks', [ExamController::class, 'marks'])->name('teacher.exams.marks');
        Route::post('/exams/marks', [ExamController::class, 'saveMarks'])->name('teacher.exams.marks.store');
    });

    Route::get('/results', [ExamController::class, 'results'])->name('teacher.results.index');
    Route::middleware(['permission:notice_view,notice_manage'])->group(function () {
        Route::get('/communication/announcements', [AnnouncementController::class, 'index'])->name('teacher.communication.announcements');
        Route::get('/communication/announcements/create', [AnnouncementController::class, 'create'])->name('teacher.communication.announcements.create');
        Route::get('/communication/announcements/{announcement}/edit', [AnnouncementController::class, 'edit'])->name('teacher.communication.announcements.edit');
        Route::post('/communication/announcements', [AnnouncementController::class, 'store'])->name('teacher.communication.announcements.store');
        Route::put('/communication/announcements/{announcement}', [AnnouncementController::class, 'update'])->name('teacher.communication.announcements.update');
        Route::delete('/communication/announcements/{announcement}', [AnnouncementController::class, 'destroy'])->name('teacher.communication.announcements.destroy');
    });
});
...
Route::middleware(['role:student'])->prefix('student')->group(function () {
    ...
    Route::get('/certificates/{id}/download', [CertificateController::class, 'studentDownload'])->name('student.certificate.download');
    Route::middleware(['permission:notice_view,notice_manage'])->group(function () {
        Route::get('/communication/announcements', [AnnouncementController::class, 'index'])->name('student.communication.announcements');
    });
});
...
Route::middleware(['role:parent'])->prefix('parent')->group(function () {
    ...
    Route::get('/attendance/grid', [AttendanceController::class, 'parentGrid'])->name('parent.attendance.grid');
    Route::middleware(['permission:homework_list'])->group(function () {
        Route::get('/homework', [HomeworkController::class, 'parentList'])->name('parent.homework.list');
    });
    Route::middleware(['permission:notice_view,notice_manage'])->group(function () {
        Route::get('/communication/announcements', [AnnouncementController::class, 'index'])->name('parent.communication.announcements');
    });
});
